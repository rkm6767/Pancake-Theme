<div class="row <?php echo ((bool) $task['completed']) ? 'completed' : 'incomplete'; ?>">
    <div class="ten columns">
		        ...
		        <div class="twelve columns mobile-four">
		            <div class="row">
		                ...
		                <div class="one column mobile-one">
  		                  <div class="task-user">
    		                    <img src="<?php echo get_gravatar(isset($task['assigned_user_email']) ? $task['assigned_user_email'] : '', '40') ?>" class="user-icon" />
		    ...
		    <div class="row task-details">
                ...
                <a class="task-icon time" title="Add Time Entry" href=""></a>
		  ...
		  <div class="two columns track-time " data-task-id="<?php echo $task['id'];?>" data-project-id="<?php echo $project->id ?>" data-time-start="<?php echo isset($task['entry_started']) && $task['entry_started'] ? strtotime(date('Y-m-d', $task['entry_started_date']).' '.$task['entry_started_time']).'000' : '' ?>">

		    <span class="time-ticker timer-time mobile-two">00:00:00</span>

		    <p class="task-buttons mobile-two">
	  		   <span class="task-btn-back btn-left"><a class="task-btn play timer-button <?php echo isset($task['entry_started']) && $task['entry_started'] ? 'running' : '' ?>" href="#" data-start="<?php echo __('global:start') ?>" data-stop="<?php echo __('global:stop') ?>" >P</a></span>
	  		   <!-- pause button (designed to replace the play button and allow user to pause the timer
	          <span class="task-btn-back btn-left"><a class="task-btn pause timer-button" href="#">P</a></span> -->
  ...
  <!-- left over from what was originally here (not replaced) -->
	<div class="row hide">
		<div class="twelve columns">
				...
				<?php if (can('create', get_client('projects', $project->id), 'project_tasks') && (!isset($task['parent_id']) || $task['parent_id'] == null)): ?>
